# Newsletter program

Taylor's newsletter system has two products. They share one conversational production process, but never share a recipient list.

## 1. Central Texas Market Update

- **Publication:** Temple TX Insider
- **Audience:** previous clients, relocation buyers, buyer leads, local homeowners, and sphere contacts with a valid permission record
- **Cadence:** first Thursday of each month
- **Promise:** the useful local market read in about three minutes, with one home, one opportunity, and one practical takeaway
- **Business goal:** remain helpful after closing, earn replies and referrals, and create buyer or seller conversations without turning the email into a sales blast

### Recurring issue structure

1. **The honest market pulse** — three to five current indicators with a plain-English interpretation.
2. **Home of the month** — chosen for story, design, neighborhood, or unusual utility, not merely the highest price.
3. **Best deal right now** — the strongest current buyer opportunity under stated criteria, including the catch.
4. **One quick tip** — a timely buyer, seller, homeowner, tax, insurance, maintenance, or relocation takeaway.
5. **The rotating extra** — one fresh human or practical feature.
6. **Reply prompt** — one easy question Taylor can answer personally.

### Six-edition creative rotation

| Edition | Rotating extra | Guardrail |
|---|---|---|
| A | Homeowner value-add project | Give a cost range and note what varies. |
| B | Local business or community shoutout | Use firsthand context; get permission for personal stories. |
| C | Neighborhood micro-spotlight | Separate facts from Taylor's judgment. |
| D | Client/home story | Get permission and omit sensitive transaction details. |
| E | Tax, insurance, or maintenance calendar | Link to the authoritative source; avoid individualized advice. |
| F | Relocation reality check | Include the tradeoff, not just the upside. |

The issue can react to the month. The rotation prevents every edition from feeling identical; it is not a requirement to force a weak feature.

## 2. Investor Deal Analysis

- **Publication:** Temple TX Investor Brief
- **Audience:** investors with a valid permission record
- **Cadence:** second and fourth Thursday of each month
- **Promise:** three active Central Texas opportunities screened under explicit assumptions, with the reason to look and the reason to walk
- **Business goal:** create serious underwriting conversations and demonstrate Taylor's ability to source, analyze, and pressure-test deals

### The three deal classes

1. **Best multifamily** — duplex, triplex, fourplex, or small multifamily.
2. **Best single-family rental** — strongest conventional long-term rental candidate.
3. **Best specialty play** — the best current value-add, flip, mid-term rental, short-term rental, land, or unusual strategy candidate.

"Best" means best among the active listings screened for that edition under the published assumptions. It does not mean guaranteed returns. If a class has no defensible pick, publish **No buy this round** and explain why instead of manufacturing a winner.

### Required analysis for every selected deal

- Active-status verification timestamp
- Price and property class
- Rent, resale, or income assumption and its source
- Financing, tax, insurance, vacancy, management, maintenance, and rehab assumptions as applicable
- Simple return or margin range—not false precision
- Why it ranks first in its class
- The largest underwriting risk
- What Taylor would verify before an offer

For short-term rentals, verify city, deed, HOA, insurance, management, and platform assumptions. For flips, state the rehab, carrying-cost, resale-cost, and ARV assumptions. Never present either strategy as permissible or profitable without the supporting checks.

### Six-edition investor rotation

| Edition | Rotating extra |
|---|---|
| A | One underwriting tip |
| B | Investor or local-operator shoutout, with permission |
| C | Value-add idea with a rough scope and risk |
| D | Financing or rate scenario |
| E | Deal post-mortem: why Taylor passed |
| F | Property-management or tenant-demand observation |

## Operating calendar

The normal system creates three sends per month without weekly list fatigue:

| Thursday | Product |
|---|---|
| First | Central Texas Market Update |
| Second | Investor Deal Analysis |
| Third | No default send; research and list care |
| Fourth | Investor Deal Analysis |
| Fifth | No default send; catch-up only with approval |

Launch exception: the first Central Texas Market Update is planned for July 23, 2026, followed by the first Investor Deal Analysis on July 30. The normal first/second/fourth-Thursday calendar begins in August.

## Evidence and compliance rules

- `/Users/taylordasch_1/market-monitor/` is the source of truth for MLS pulls.
- Date-stamp every market snapshot and recheck listing status before staging and again before sending.
- Use ranges when assumptions are uncertain and label estimates clearly.
- Do not expose confidential MLS remarks, showing instructions, or private client information.
- Shoutouts and client stories require permission.
- Keep a concise disclaimer on investor editions: information is educational, assumptions vary, and readers should verify financing, taxes, insurance, condition, rents, restrictions, and professional advice.
- One primary CTA per issue. A reply is the preferred conversion event.

## Production conversation

For each edition, Taylor only needs to answer four prompts here:

1. What happened this month—or what are investors asking right now?
2. Is there a property, person, tip, or local story Taylor already wants featured?
3. What action should the reader take: reply, request the deal sheet, view listings, or book a call?
4. Is there anything Taylor explicitly does not want mentioned?

Codex then pulls the supporting data, drafts the issue, produces the local preview, and presents the subject line, recipient count, claims, links, and assumptions for approval. Staging and sending remain separate approvals.
